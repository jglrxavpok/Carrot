...
namespace Peeler::ECS {
    ...
    void LightEditorRenderer::onFrame(const Carrot::Render::Context& renderContext) {
        ...
        forEachEntity([&](Carrot::ECS::Entity& entity, Carrot::ECS::TransformComponent& transform, Carrot::ECS::LightComponent& lightComponent) {
            ...
            const auto& textureHandle = (lightHandle->flags & Carrot::Render::LightFlags::Enabled) != Carrot::Render::LightFlags::None ? textureHandleOn : textureHandleOff;
            billboardRenderer.render(transform.computeFinalPosition(), scale, *textureHandle, renderContext, lightHandle->color, entity.getID());
        });
    }

...
void Carrot::Render::PassBase::addInput(const Carrot::Render::FrameResource& resource) {
...
std::unique_ptr<Carrot::Render::CompiledPass> Carrot::Render::PassBase::compile(Carrot::VulkanDriver& driver, Carrot::Render::Graph& graph) {
    ...
    return make_unique<CompiledPass>(driver, std::move(framebuffers), std::move(renderPass), clearValues, generateCallback());
}

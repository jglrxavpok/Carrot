...
namespace Tools {
    ...
    public:
        ...
        void visitGetVariable(Carrot::GetVariableExpression& expression) override {
            ...
            if(var == VariableNode::getInternalName(VariableNodeType::GetSize)) {
                ...
                ids.push(vars.deltaTime);
            } else if(var == VariableNode::getInternalName(VariableNodeType::GetParticleID)) {
                ids.push(vars.particleID);
            } else if(var == VariableNode::getInternalName(VariableNodeType::GetPosition)) {
                auto access = builder.createAccessChain(spv::StorageClassStorageBuffer, vars.position, {builder.makeIntConstant(expression.getSubIndex())});
                ids.push(builder.createLoad(access, spv::NoPrecision));
            } else {
    ...
    std::vector<uint32_t> ParticleShaderGenerator::compileToSPIRV(const std::vector<std::shared_ptr<Carrot::Expression>>& expressions) {
        ...
        builder.addDecoration(gl_WorkGroupSize, spv::DecorationBuiltIn, spv::BuiltIn::BuiltInWorkgroupSize);
        {
            ...
            Function* mainFunction = builder.makeFunctionEntry(spv::NoPrecision, builder.makeVoidType(), "main", {}, precisions, &functionBlock);
            ...
            auto particleID = builder.createLoad(varParticleIndex, spv::NoPrecision);
            ...
            {
                auto shouldContinue = builder.createBinOp(spv::OpUGreaterThanEqual, builder.makeBoolType(), particleID, particleCount);
            ...
            auto velocityAccess = builder.createAccessChain(spv::StorageClassStorageBuffer, descriptorSet, {builder.makeIntConstant(0), particleID, builder.makeIntConstant(2)});
            auto positionAccess = builder.createAccessChain(spv::StorageClassStorageBuffer, descriptorSet, {builder.makeIntConstant(0), particleID, builder.makeIntConstant(0)});
            ...
            auto lifeAccess = builder.createAccessChain(spv::StorageClassStorageBuffer, descriptorSet, {builder.makeIntConstant(0), particleID, builder.makeIntConstant(1)});
            ...
            auto sizeAccess = builder.createAccessChain(spv::StorageClassStorageBuffer, descriptorSet, {builder.makeIntConstant(0), particleID, builder.makeIntConstant(3)});

            auto castParticleID = builder.createUnaryOp(spv::OpConvertUToF, float32Type, particleID);

            // user-specific code

            {
                auto expr = Carrot::CompoundExpression(expressions);
                builder.setSourceText(expr.toString());
            }
            Variables vars{};
            vars.deltaTime = dt;
            ...
            vars.particleID = castParticleID;

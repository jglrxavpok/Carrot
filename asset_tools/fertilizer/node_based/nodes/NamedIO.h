...
namespace Fertilizer {
    ...
    public:
        explicit NamedIONode(Fertilizer::EditorGraph& graph): EditorNode(graph, isInput ? "Named Input" : "Named Output", isInput ? "named_input" : "named_output") {
            name = isInput ? "NamedInput" : "NamedOutput";
            load();
        }

        explicit NamedIONode(Fertilizer::EditorGraph& graph, const rapidjson::Value& json): EditorNode(graph, isInput ? "Named Input" : "Named Output", isInput ? "named_input" : "named_output", json) {
    ...
    public:
        ...
        bool renderCenter() override {
            ...
            ImGui::SetNextItemWidth(200);
            if(Fertilizer::NodeEditor::BeginNodeCombo("Type##template_node_type", selectableTypes[selectedTypeIndex].name().c_str(), 0)) {
                ...
                Fertilizer::NodeEditor::EndNodeCombo();
            }

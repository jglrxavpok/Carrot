...
#include "node_based/nodes/NamedIO.h"
...
namespace Fertilizer {
    ...
    void TemplateNode::loadInternalGraph(const rapidjson::Value& graphJson) {
        internalGraph = std::make_unique<EditorGraph>("template_internal_" + title);
        {
            NodeLibraryMenuScope s1("Operators", internalGraph.get());
            Fertilizer::Nodes::addCommonOperators(*internalGraph);
        }
        {
            NodeLibraryMenuScope s1("Logic", internalGraph.get());
            Fertilizer::Nodes::addCommonLogic(*internalGraph);
        }

        {
            NodeLibraryMenuScope s1("Functions", internalGraph.get());
            Fertilizer::Nodes::addCommonMath(*internalGraph);
        }

        {
            NodeLibraryMenuScope s1("Inputs", internalGraph.get());
            Fertilizer::Nodes::addCommonInputs(*internalGraph);
        }

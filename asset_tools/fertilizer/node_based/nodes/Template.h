...
#define THROW_IF(condition, message) if(condition) throw TemplateParseError(message, templateName, __FILE__, __FUNCTION__, __LINE__, #condition);
    public:
        explicit TemplateNode(Fertilizer::EditorGraph& graph, const std::string& name): EditorNode(graph, name, "template"), templateName(name) {
            load();
        }

        explicit TemplateNode(Fertilizer::EditorGraph& graph, const rapidjson::Value& json): EditorNode(graph, "tmp-template", "template", json) {

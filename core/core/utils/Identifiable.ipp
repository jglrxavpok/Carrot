...
const Carrot::ComponentID Carrot::Identifiable<Type>::getID() {
    ...
    std::call_once(flag, [](Carrot::ComponentID componentID) {
        Carrot::GetIdentifiableNames().getOrCompute(id, []() { return getStringRepresentation(); });
        Carrot::GetIdentifiableIDs().getOrCompute(getStringRepresentation(), [idCopy = id]() { return idCopy; });
    }, id);
    return id;
}
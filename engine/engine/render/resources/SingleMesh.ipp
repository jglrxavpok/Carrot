...
template<typename VertexType>
Carrot::SingleMesh::SingleMesh(const Carrot::Vector<VertexType>& vertices, const Carrot::Vector<std::uint32_t>& indices): Carrot::Mesh::Mesh() {
    sizeofVertex = sizeof(VertexType);
    const auto& queueFamilies = GetVulkanDriver().getQueuePartitioning();
    // create and allocate underlying buffer
    std::set<std::uint32_t> families = {
    ...
    // upload vertices
    vertexAndIndexBuffer->stageUploadWithOffsets(std::make_pair(vertexStartOffset, std::span<const VertexType>(vertices.data(), static_cast<std::size_t>(vertices.size()))), std::make_pair(static_cast<uint64_t>(0), std::span<const u32>(indices.data(), static_cast<std::size_t>(indices.size()))));
}